...
function renderTodaySchedule() {
  ...
  if (todaySchedules.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><div class="empty-icon"></div><div class="empty-text">No schedule for today</div></div>';
    return;
  }
...
function renderUpcomingDeadlines() {
  ...
  if (upcoming.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><div class="empty-icon">✅</div><div class="empty-text">No upcoming deadlines</div></div>';
    return;
  }
...
function renderSubjects() {
  ...
  if (subjects.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><div class="empty-icon">📚</div><div class="empty-text">No subjects yet</div><p style="color: var(--text-secondary);">Add your first subject to get started</p></div>';
    return;
  }
...
function editSubject(id) {
  ...
  if (subject) {
    document.getElementById("addSubjectModal").classList.add("active");
    document.querySelector("#addSubjectModal .modal-title").textContent =
      "Edit Subject";
...
function renderSchedules() {
  ...
  if (daySchedules.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><div class="empty-icon">📅</div><div class="empty-text">No schedule for ' +
      currentDay +
      "</div></div>";
    return;
  }

  container.innerHTML = daySchedules
    .map((schedule) => {
      const subject = subjects.find((s) => s.id === schedule.subjectId);
      return `
                    <div class="schedule-item">
...
function renderTasks() {
  ...
  if (tasks.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><div class="empty-text">No tasks yet</div></div>';
    return;
  }
...
function renderAnalytics() {
  ...
  chartContainer.innerHTML = subjects
    .map((subject) => {
